...
class GraphFilterBatchAttentional(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.concatenate:
            ...
            # and then over p: (p=0,f=0), (p=0,f=1), ..., (p=0,f=F-1), (p=1,f=0), (p=1,f=1), ..., etc.
            y = y.permute(0, 3, 1, 2)\
...
# self.filterWeight, x, self.mixer, self.weight, self.weight_bias, self.S, b=self.bias
